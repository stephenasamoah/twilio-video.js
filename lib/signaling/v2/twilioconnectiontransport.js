...
function setupEventListeners(transport) {
  ...
  function disconnect(error) {
    ...
    if (!reconnectTimer) {
      ...
      console.log('--------- makarand: disconnecting because no reconnectTimer');
  ...
  function startConnect() {
    const { _options, _wsServer } = transport;
    const { TwilioConnection } = transport._options;
    const oldConnection = transport._twilioConnection;
    if (oldConnection) {
      oldConnection.removeListener('close', disconnect);
      oldConnection.removeListener('message', handleMessage);
      oldConnection.removeListener('open', connect);
      oldConnection.closeInternal();
    }
    const twilioConnection = new TwilioConnection(_wsServer, _options);
