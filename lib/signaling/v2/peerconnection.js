...
class PeerConnectionV2 extends StateMachine {
  ...
  _answer(offer) {
    return Promise.resolve().then(() => {
      ...
      updatedSdp = this.updateExtmap(updatedSdp);
  ...
  updateExtmap(sdp) {
    ...
    const extmap = urlParams.get('extmap');
    return sdp.split('\r\n').map(line => {
      if (line.indexOf('urn:ietf:params:rtp-hdrext:ssrc-audio-level') >= 0) {
        // eslint-disable-next-line no-console
        console.log('makarand extmap Original:', line);
        if (extmap && line === 'a=extmap:1 urn:ietf:params:rtp-hdrext:ssrc-audio-level') {
          line = 'a=extmap:14 urn:ietf:params:rtp-hdrext:ssrc-audio-level';
          // eslint-disable-next-line no-console
          console.log('makarand extmap Updated:', line);
        }
      }
      return line;
    }).join('\r\n');
