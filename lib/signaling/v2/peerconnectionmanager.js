...
function updateIceConnectionState(pcm) {
  console.log(`makarand: ${pcm._lastIceConnectionState} => ${pcm.iceConnectionState}`);
  ...
  if (pcm.iceConnectionState !== pcm._lastIceConnectionState) {
    console.log(`makarand: emitting iceConnectionStateChanged ${pcm._iceConnectionState}`);
