...
class PeerConnectionManager extends QueueingEventEmitter {
  ...
  constructor(iceServerSource, encodingParameters, preferredCodecs, options) {
    super();
    console.log('makarand: I am peerConnection Manager');
    ...
    const log = options.log
      ? options.log.createLog('PCM', this)
      : new Log('PCM', this, logLevels);
    ...
    this._log.debug('makarand: and I can debug!');
    this._log.debug('makarand: and I can info!');
    this._log.warn('makarand: and I can warn!');
  }

  toString() {
    return `[PCM #${this._instanceId}]`;
  }
  ...
  _getOrCreate(id, configuration) {
    this._log.warn('makarand: _getOrCreate');
    const self = this;
    ...
    if (!peerConnection) {
      ...
      peerConnection.on('iceConnectionStateChanged', caller => {
        this._log.info('makarand: 1 iceConnectionStateChanged: caller = ', caller);
...
function updateIceConnectionState(pcm) {
  pcm._log.warn(`makarand: ${pcm._lastIceConnectionState} => ${pcm.iceConnectionState}`);
  pcm._lastIceConnectionState = pcm.iceConnectionState;
  [...pcm._peerConnections.values()].forEach(pcv2 => pcm._log.info('makarand: iceConnectionState : ', pcv2.iceConnectionState));
  pcm._iceConnectionState = summarizeIceOrPeerConnectionStates(
    [...pcm._peerConnections.values()].map(pcv2 => pcv2.iceConnectionState));

  pcm._log.warn('summarizeIceOrPeerConnectionStates : ', pcm._iceConnectionState);
  if (pcm.iceConnectionState !== pcm._lastIceConnectionState) {
    pcm._log.warn(`makarand: emitting iceConnectionStateChanged ${pcm._iceConnectionState}`);
